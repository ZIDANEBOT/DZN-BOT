...
// Dilarang hapus credits, Atau Zdbot Akan Marah
//
//
...
module.exports = zfa = async (zfa, message) => {
    try {
        ...
        const ownerNumber = '625895413910@c.us'
		...
		// Biar Dek Zdbot Baca Chatt Secara Otomatis
         zfa.sendSeen(chatId)
 ...
 const jwb = [
            ...
            'apa?',
            'iyaa',
            'halo kak, Zdbot disini',
            'kenapa kak',
			'kenapa manggil-manggil? kangen ya... ketik #menu aja kak',
			'ada apa manggil Zdbot? kangen ya... ketik #menu aja',
			'Apa sayang?',
			'Ada apa sayang?'
        ]
	...
	const sotoy = [
        ...
        const mess = {
            wait: 'Bentar ya kak lagi Zdbot proses nih. Mohon tunggu sebentar...',
        ...
        function isStickerMsg(id){
            ...
            for (let i of stickerspam){
                if(i.id === id){
                    if (i.msg >= 12) {
                        found === true 
                        zfa.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh Zd bot', message.id).then(() => {
        ...
        function isBadwordMsg(id){
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 12) { // 12x
                        kasar === true 
                        zfa.reply(from, '*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh Zdbot!', message.id).then(() => {
                            zfa.removeParticipant(groupId, id)
                        }).then(() => {
                            const cus = id
                            var found = false
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
		...
		if (args.includes('zidan')){
			...
			const txt = body.slice(0)
             zfa.reply(from, 'HAYO LAGI NGOMONGIN OWNER YAA!! Zdbot CEPUIN AH:D', id)
		...
		if (args.includes('terima kasih') || args.includes('makasih') || args.includes('terimakasih') || args.includes('thank you') || args.includes('thanks')) {
            zfa.reply(from, `sama-sama ${pushname}💖`, id)
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return zfa.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin zdbot!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return zfa.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner Zdbot!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return zfa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Zdbot!', id)
		...
		case prefix+'nightcore':
                ...
                if(cekumur(cekage)) return
                if (isMedia && isAudio || isQuotedAudio || isVoice || isQuotedVoice) {
                    await zfa.reply(from, 'Bentar ya Tod lagi Zdbot proses nih, mohon tunggu sebentar....', id)
                    const encryptMedia = isQuotedAudio || isQuotedVoice ? quotedMsg : message
                    console.log(color('[WAPI]', 'green'), 'Downloading and decrypting media...')
                    const mediaData = await decryptMedia(encryptMedia, uaOverride)
                    const temp = './temp'
...
➸ Biodata : ${Biodata}`
        ...
        case prefix+'addbadword':
            if (!isAdmin) return zfa.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Zdbot!`, id)
        ...
        case prefix+'listbadword':
            if (!isAdmin) return zfa.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin Zdbot!`, id)
                ...
                let nomre = 1
                    ...
                    break
		case prefix+'getses':
            if (!isOwner) return zfa.reply(from, 'Perintah ini hanya untuk Owner Zdbot', id)            
            const sesPic = await zfa.getSnapshot()
            zfa.sendFile(from, sesPic, 'session.png', 'Nih boss', id)
            break
        case prefix+'Zdbotadmin':
            let admn = `This is list of Zdbot Admin\nTotal : ${adminNumber.length}\n`
		...
		case prefix+'ban':
           if (!isOwner) return zfa.reply(from, 'FItur ini hanya untuk Zidan Tersayang :*', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return zfa.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin Zdbot!`, id)
        ...
        case prefix+'listblock':
            ...
            break
		case prefix+'gift': // Hanya Admin & Owner zdbot yang bisa gift Limit
            if (!isAdmin, !isOwner) return zfa.reply(from, `Maaf, perintah ini hanya dapat dilakukan oleh Admin Zdbot!`, id)
                    ...
                    if(jmla > 9999999999999999999) return await zfa.reply(from, `Maximal  9999999999999999999!`, id)
                        ...
                        if (found !== false) {
                            ...
                            if(limit[found].limit <= 20) return zfa.re ply(from, `Kuota Limit pada nomor tersebut masih penuh\nUntuk gift pastikan kuota limit target sudah habis`, id)
...
• Limit: ${limitCount-updated.limit}`
		...
		case prefix+'addadmin':
            if (!isOwner) return zfa.reply(from, 'FItur ini hanya untuk Zidan Tersayang :*', id)
                ...
                zfa.reply(from, 'Success Menambahkan Admin Zdbot!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return zfa.reply(from, 'FItur ini hanya untuk Zidan Tersayang :*', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                zfa.reply(from, 'Success Menghapus Admin Zdbot!', id)
            break
        case prefix+'block':
            if (!isOwner) return zfa.reply(from, 'FItur ini hanya untuk Zidan Tersayang :*', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
		...
		case prefix+'take':
                    if (quotedMsg && quotedMsg.type == 'sticker' || quotedMsg && quotedMsg.type == 'image') {
                        argz = body.trim().split('|') //zfa.reply(from, 'Contoh #take |Zdbot Punya|ZidanGanzz', id)
                        await zfa.reply(from, mess.wait, id)
		...
		case prefix+'toimg':
            ...
            if (quotedMsg && quotedMsg.type == 'sticker') {
                const mediaData = await decryptMedia(quotedMsg)
                zfa.reply(from, `Bentar ya tod lagi Zdbot proses nih mohon tunggu sebentar....`, id)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await zfa.sendFile(from, imageBase64, 'imagesticker.jpg', 'Yeay! Berhasil.... Kak Zdbot minta tolong subscribe channel youtube owner aku: https://youtube.com/zidanfadilaharsazfaa', id)
        ...
        case prefix+'sgif': // MRHRTZ
            zfa.reply(from, `Bentar ya kak lagi Zdbot proses nih mohon tunggu sebentar....`, id)
        ...
        case prefix+'sider':
            if (!isGroupMsg) return zfa.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return zfa.reply(from, `Tolong Reply Pesan Zdbot`, id)
            if (!quotedMsgObj.fromMe) return zfa.reply(from, `Tolong Reply Pesan Zdbot`, id)
            try {
                ...
                zfa.sendTextWithMentions(from, `Ciee, Ngeread... Suka Sama Zdbot Yah....\n${list}`)
            } catch(err) {
                console.log(err)
                zfa.reply(from, `Maaf, Belum Ada Yang Membaca Pesan Zdbot atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
		...
		case prefix+'join':
           ...
           // if (!isOwner) return zfa.reply(from, 'Fitur Ini Hanya Untuk Zidan Tersayang :*', id)
            ...
            if (check.status === 200) {
                await zfa.joinGroupViaLink(link).then(() => zfa.reply(from, 'Zdbot Akan Masuk Kak. Terimakasih telah mengundang Zdbot'))
            } else {
...
• Plan to Read: ${jikan.manga_stats.plan_to_read}`
        ...
        case prefix+'simi':
            ...
            if (!isAdmin) return zfa.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin Zdbot!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return zfa.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
		...
		case prefix+'kenalan':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 80) return await zfa.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan Zdbot', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
            ...
            case prefix+'kenalulang':
                    if (!isAdmin) return zfa.reply(from, 'Command ini hanya dapat digunakan oleh admin Zdbot', id)  
...
➸ *Wilayah:* +${region_name}`
		...
		case prefix+'cewek':
            ...
            zfa.sendFileFromUrl(from, pep, 'pptl.jpg', 'Suppor Zdbot Dengan Cara Follow IG: https://instagram.com/_zidanfadilaharsa Dan Subscribe YT: https://youtube.com/zidanfadilaharsazfaa', message.id)
            await limitAdd(serial)
            break
        case prefix+'neko':
            if(isReg(obj)) return
		...
		case 'sa':
                    ...
                    break
	    case prefix+'linkZdbot':
		   if(isReg(obj)) return
            if(cekumur(cekage)) return
			zfa.sendText(from, 'Ini link nomor Zdbot Kak https://wa.me/447537102515 share ke teman kakak juga yah....', id)
		case prefix+'menu':
        case prefix+'help':
		case prefix+'Zdbotmenu':
		    if(isReg(obj)) return
            if(cekumur(cekage)) return
		...
		case prefix+'changelog':
			...
			break
		case prefix+'runtime':
            zfa.reply(from, `Zdbot telah aktif selama :\n${cts}`, id)
            break
		case prefix+'donate':
		case prefix+'donasi':
		case prefix+'bantuZdbot':
        ...
        case prefix+'bahasa':
            ...
            break
		case prefix+'Zdbotagroup':
		case prefix+'Zdbotgc':
            zfa.reply(from, `Link Group Zdbot Fariza Cans : https://chat.whatsapp.com/D4L2QeBc9B0IXx2vGQI02p\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
	    case prefix+'yourpic':
		case prefix+'pplu':
                    if(isReg(obj)) return
            if(cekumur(cekage)) return
                    ...
                    zfa.sendFileFromUrl(from, ypfp, 'pfpy.jpg', `Support Zdbot Dengan Cara Follow Ig: https://instagram.com/_zidanfadilaharsa Dan Subscribe Channel Yt https://youtube.com/zidanfadilaharsazfaa`)
                    break
		case prefix+'play':
                misc.ytPlay(q)
                    .then(async ({ result }) => {
		...
		case prefix+'ppgua':  
            ...
            if(cekumur(cekage)) return
                    ...
                    zfa.sendFileFromUrl(from, mpfp, 'pfpm.jpg', `Support Zdbot Dengan Cara Follow Ig: https://instagram.com/_zidanfadilaharsa Dan Subscribe Channel Yt https://youtube.com/zidanfadilaharsazfaa`)
                    break
		case prefix+'profile':
		case prefix+'aku':
            if(isReg(obj)) return
        ...
        case prefix+'wame':
            ...
            break
			        default:
            //if (!isGroupMsg) return zfa.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup Zdbot, Link Ada Di Bio atau Bisa Mengetik #Zdbotgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
            if (command.startsWith('#')) {
                zfa.reply(from, `Maaf kak ${pushname}, Command *${args[0]}* Tidak Terdaftar Di Dalam *#menu*!`, id)
            }
			/*if (command.startsWith('!')) {
                zfa.reply(from, `Maaf kak ${pushname} Sayang, Zdbot pake prefix # bukan *!* coba deh kakak ketik *#menu*`, id)
            }
			if (command.startsWith('.')) {
                zfa.reply(from, `Maaf kak ${pushname} Sayang, Zdbot pake prefix # bukan *.* coba deh kakak ketik *#menu*`, id)
            }
			...
			if (command.startsWith('x')) {
                zfa.reply(from, `Maaf kak ${pushname} Sayang, Zdbot pake prefix # bukan *x* coba deh kakak ketik *#menu*`, id)
            }*/
            await zfa.sendSeen(from) 
            }
        }
