...
const start = async (zfa = new Client()) => {
        console.log('------------------------------------------------')
        lolcatjs.fromString(color(figlet.textSync('Zidanbot', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        lolcatjs.fromString('[DEV] ZIDANBOT')
        ...
        zfa.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return zfa.sendText(chat.id, 'Halo aku zdBot, Ketik #help Untuk Melihat List Command Ku...')
            if(mtcState === false){
                const groups = await zfa.getAllGroups()
                // BOT group count less than
                if(groups.length > groupLimit){
                    await zfa.sendText(chat.id, 'Maaf, Batas group yang dapat Zdbot tampung sudah penuh').then(async () =>{
                        zfa.deleteChat(chat.id)
                        zfa.leaveGroup(chat.id)
                    })
                }else{
                    if(chat.groupMetadata.participants.length < memberLimit){
                        await zfa.sendText(chat.id, `Maaf, Zdbot keluar jika member group tidak melebihi ${memberLimit} orang`).then(async () =>{
                            zfa.deleteChat(chat.id)
                            zfa.leaveGroup(chat.id)
                        })
                    }else{
                        if(!chat.isReadOnly) zfa.sendText(chat.id, 'Halo aku zdbot, Ketik #help Untuk Melihat List Command Ku...')
                    }
                }
            }else{
                await zfa.sendText(chat.id, 'Zdbot sedang maintenance, coba lain hari').then(async () => {
                    zfa.deleteChat(chat.id)
                    zfa.leaveGroup(chat.id)
                })
            }
